...
centroid_total = pd.DataFrame(columns=list(centroids['aeroplane']))
for keys, values in centroids.items():
    centroid_total = pd.concat([centroid_total,values])
    
#Create k-means model with the total centroids
kmeans_total = KMeans(init=centroid_total,n_clusters=len(centroid_total),
                      n_init=1).fit(centroid_total)

#Predict on images
centroids_total = kmeans_total.predict(pca_conca)
